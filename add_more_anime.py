...
def load_anime(conn, genres, limit=50):
    print(f"\n[2/4] Loading next {limit} anime...")

    existing_external_ids = set()
    cursor = conn.cursor()
    cursor.execute("SELECT external_id FROM anime")
    for row in cursor.fetchall():
        existing_external_ids.add(row[0])
    ...
    while len(all_anime) < limit:
        ...
        for anime in data["data"]:
            mal_id = anime["mal_id"]
            if mal_id not in existing_external_ids:
                all_anime.append(anime)
                existing_external_ids.add(mal_id)
    ...
    print(f"  Fetched {len(all_anime)} new anime entries")
    if not all_anime:
        return []

    print("\n[3/4] Saving anime to database...")
    saved_ids = []
...
def main():
    print("=" * 60)
    print("ANIMEHAKO DATABASE - ADDING 50 MORE ANIME")
    print("=" * 60)
    ...
    try:
        ...
        print("\n" + "=" * 60)
        print(f"POPULATION COMPLETE! Total anime in DB: {count}")
        print("=" * 60)
    except Exception as e:
        print(f"\nError: {e}")
        conn.rollback()
